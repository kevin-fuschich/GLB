...
For a 162-game season, 75% targets approximately 122 games per club.

The backfill engine:
- preserves existing official games
- counts games already played by each club
- schedules only the games needed to reach the target
- favors less-used opponents
- balances home and away assignments
- generates rich `glb.boxscore.v1` files through the same simulator used for daily games
- rebuilds derived records after generation

Preview without writing games:

```bash
node tools/backfill-season.js --percent 75 --dry-run
```

The 75% dry-run, JavaScript syntax checks, and canonical roster validation passed in GitHub Actions on the engine branch before generation was enabled.
...
- `data/schedule.json` (legacy presentation stub)

`data/days/` is a presentation/index layer and must not override official box scores.
